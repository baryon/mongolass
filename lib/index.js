...
module.exports = class Mongolass {
  ...
  async connect (url, opts) {
    ...
    this._client = await mongodb.MongoClient.connect(this._url, this._opts)
   ...
   * get/set gridfs
   */
  gridfs(bucketName='fs', opts={}) {
    if(!opts.bucketName){
      opts.bucketName = bucketName
    }
    this._gridfs[opts.bucketName] = this._gridfs[opts.bucketName] || new GridFS(this, opts)

    return this._gridfs[opts.bucketName]
  }
